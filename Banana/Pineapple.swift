...
class Pineapple: UIViewController {
    struct Pineapples {
        var one : Int
        var two : Int
        var javob : Int{
    ...
    var array : [Pineapples] = []
    var joriy_savol = 0
    var hisob_next = 0
    var natija_hisob = 0
    var over_hisob = 0
    
    var bir = Pineapples(one: 2, two: 2, variantlar: [4,5,1,7])
    var ikki = Pineapples(one: 3, two: 3, variantlar: [3,1,5,6])
    var uch = Pineapples(one: 3, two: 1, variantlar: [10,4,9,2])
    var tort = Pineapples(one: 1, two: 3, variantlar: [4,3,1,7])
    var besh = Pineapples(one: 1, two: 1, variantlar: [2,5,1,9])
    var olti = Pineapples(one: 3, two: 2, variantlar: [4,5,11,71])
    var yetti = Pineapples(one: 2, two: 2, variantlar: [4,5,11,7])
    var sakkiz = Pineapples(one: 1, two: 2, variantlar: [3,5,1,7])
    ...
    @IBAction func answer_Pressed(_ sender: UIButton) {
        ...
        if Int(answer[sender.tag].currentTitle!) == array[joriy_savol].javob{
            ...
            UIView.animate(withDuration: 0.001){
                self.animate.transform = .init(scaleX: 1, y: 0.001)
            }completion: { (_) in
